...
bool fixdump::current::GetWoWPeHeader(BUFFERED_PE_HEADER& PeHeader)
{
    ...
    if (!GetModuleFileNameExW(debuggee.hProcess,
                              ...
                              WoWPath,
                              MAX_MODULE_SIZE)) {
        pluginLog("Error: failed to get World of Warcraft's path. %d \n", GetLastError());
        return false;
    }
    ...
    if (hWoWFile == INVALID_HANDLE_VALUE) {
        pluginLog("Error: failed to open WowClassic.exe while getting PE header. %d \n", GetLastError());
        return false;
    }
    ...
    if (!ReadFile(hWoWFile,
                  ...
                  &numBytesRead,
                  nullptr)) {
        pluginLog("Error: failed to read WowClassic.exe.\n");
...
void fixdump::current::FixPeHeader(BUFFERED_PE_HEADER& PeHeader)
{
    ...
    debuggee.imageSize = (PIMAGE_OPTIONAL_HEADER(&PeHeader.ntHeaders->OptionalHeader))->SizeOfImage;
}
...
SIZE_T BuildNewWoWRegion(const REMOTE_PE_HEADER& WoWPEHeader)
{
    ...
    if (const SIZE_T secretPEHeaderAddress = GetSecretPEHeaderBaseAddress())
    {
                                 ...
                                 WoWPEHeader.optionalHeader->SizeOfImage))
        {
            ...
            pluginLog("BuildNewWoWRegion: Failed to write transfer buffer to WowClassic.exe.\n");
    }
    else
...
bool NoticeMeScylla(const REMOTE_PE_HEADER& NewRegionPEHeader)
{
    ...
    // read WowClassic.exe's LDR_DATA_TABLE_ENTRY.
